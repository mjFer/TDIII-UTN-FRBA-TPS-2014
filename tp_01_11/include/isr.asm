...
;-----------------------------------------------------------
; Handler de Interrupcion de timer
; Recibe: Nada
; Devuelve: Nada
;------------------------------------------------------------
%define TASK_IDLE  0
...
CurrentTask     db TASK_IDLE      ; contiene la tarea actual 
OldTSSAddr      dq TaskIdle
NewTSSAddr      dq Task_B
...
int0_TimerHandler:
    ...
    cmp byte[CurrentTask],TASK_IDLE                 
    je  test_status_idle
    cmp byte[CurrentTask],TASK_A
    jne test_status_Inf
    jmp test_status_Sup
...
test_status_Sup:            
     ;mov eax,[TaskSup+task_struct.CurrentTicks]
     ;inc eax
     ;mov [TaskSup+task_struct.CurrentTicks],eax
     ;cmp eax,[TaskSup+task_struct.MaxTicks]
     ;jne fin_irq
     mov byte[CurrentTask],TASK_B
     mov rax,TaskMalloc
     mov [OldTSSAddr],rax
     mov rax,Task_B
     mov [NewTSSAddr],rax
     ;mov dword[TaskSup+task_struct.CurrentTicks],0
     jmp SwitchTo

test_status_Inf:                ;Evaluamos si debemos seguir con la tarea 2
     ;mov eax,[TaskInf+task_struct.CurrentTicks]
     ;inc eax
     ;mov [TaskInf+task_struct.CurrentTicks],eax
     ;cmp eax,[TaskInf+task_struct.MaxTicks]
     ;jne fin_irq
     mov byte[CurrentTask],TASK_IDLE
     mov rax,Task_B
     mov [OldTSSAddr],rax
     mov rax,TaskIdle
     mov [NewTSSAddr],rax
     ;mov dword[TaskInf+task_struct.CurrentTicks],0
     jmp SwitchTo

test_status_idle:              ;Evaluamos si debemos seguir con la tarea idle
     ;mov eax,[TaskIdle+task_struct.CurrentTicks]
     ;inc eax
     ;mov [TaskIdle+task_struct.CurrentTicks],eax
     ;cmp eax,[TaskIdle+task_struct.MaxTicks]
     ;jne fin_irq
     mov byte[CurrentTask],TASK_A
     mov rax,TaskIdle
     mov [OldTSSAddr],rax
     mov rax,TaskMalloc
     mov [NewTSSAddr],rax
     ;mov dword[TaskIdle+task_struct.CurrentTicks],0
     jmp SwitchTo
