...
; Funcion get priority
; Argumentos:
;	rdi : milisegundos a dormir	
; Devuelve: los ms
; Autor: Marcelo J Fernandez
;------------------------------------------------------------	        
mSleep:
  mov rbx,[Current_disp_Task]   
  mov [rbx + dispatcher_task_struct.sleepMS],rdi		;guardo los ms que va a dormir la tarea
  mov word [rbx + dispatcher_task_struct.state]  ,TASK_DORMANT	;seteo el flag de dormida
  call Sch_switchTask						;llamo al cambio de tarea 
  ret
    
;-----------------------------------------------------------
; Funcion de inicializacion de SIMD
; Devuelve: nada
; Autor: Marcelo J Fernandez
;------------------------------------------------------------	  
%define SIMD_OSXMMEXCPT  1<<10 ;Enable SIMD floating Point Exeptions
%define SIMD_OSFXSR  1<<9 ;Enable SIMD floating Point Exeptions
%define CR0_TS 1<<3

SIMD_Init:
  push rax
  mov rax,CR4
  or rax, SIMD_OSXMMEXCPT
  or rax, SIMD_OSFXSR    
  mov CR4, rax   
